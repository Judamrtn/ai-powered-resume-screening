...
IMPORTANT: Route ordering matters in FastAPI — more specific paths
...
def generate_calendar_event(interview: Interview, candidate: Candidate, job: Job) -> CalendarEvent:
    ...
    title        = f"{format_label} — {candidate.name} for {job.title}"
    description  = (
        f"Interview with {candidate.name} ({candidate.email})\n"
        f"Position: {job.title} — {job.department or ''}\n"
...
# (must be declared BEFORE /{interview_id} routes — more specific path first)
# ════════════════════════════════════════════════════════════════════════════
...
def download_ical(
    ...
    """Download iCal file for the interview — can be imported into any calendar app."""
...
def submit_feedback(
    ...
    """Submit post-interview feedback. No auth required — interviewers submit directly."""
    interview = get_interview_or_404(interview_id, job_id, db)
